...
//                }
    ...
    public ResponseUtil updateConnectionDetails(UserConnectorConfigDto userConnectorConfigDto) {
        ResponseUtil responseUtil = new ResponseUtil();
        DbConnectionDetails dbConnectionDetails = dbConnectionDetailsRepository.findByConnectionName(userConnectorConfigDto.getConnectionName());
        if (dbConnectionDetails == null) {
            responseUtil.setMessage("No record found in the Db for id = " + userConnectorConfigDto.getId());
            responseUtil.setStatusCode(RestResponse.StatusCode.NO_CONTENT);
            return responseUtil;
        }
        userConnectorConfigDto.setId(dbConnectionDetails.getId());
        DbConnectionDetails dbConnectionDetailsNew = DtoHelper.convertToDbConnectionDetails(userConnectorConfigDto);
        try {
            if (userConnectorConfigDto.getConnector().equals(ConnectorEnum.MYSQL.toString())) {
        ...
        dbConnectionDetailsNew.setDbPassword(encryptionService.encrypt(dbConnectionDetailsNew.getDbPassword()));
        dbConnectionDetailsRepository.update(dbConnectionDetailsNew);
        responseUtil.setStatusCode(RestResponse.StatusCode.OK);
        responseUtil.setMessage("Updated connection Details...");
        return responseUtil;
    }

    public List<UserConnectorConfigDto> getConnectionByMiniAppName(String miniapp) {
        List<UserConnectorConfigDto> response  = new ArrayList<>();
        List<DbConnectionDetails> response1 = dbConnectionDetailsRepository.findByMiniAppName(miniapp);
